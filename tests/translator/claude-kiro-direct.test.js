...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("embeds Claude system instructions in content without extra wire fields", () => {
    const out = C2K({
    ...
    expect(out.conversationState.currentMessage.userInputMessage).not.toHaveProperty("systemInstruction");
    expect(contentOf(out)).toContain("system-only instruction");
  });

  it("preserves system instructions in the replayed first turn", () => {
    const credentials = {
      rawHeaders: { "x-session-id": "claude-system-instruction-replay" },
      connectionId: "kiro-account-system-instruction",
    };
    const first = C2K({
    ...
    expect(second.conversationState.history[0].userInputMessage.content).toBe(contentOf(first));
